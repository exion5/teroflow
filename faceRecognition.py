import cv2
import time
from insightface.app import FaceAnalysis

app = FaceAnalysis(name='buffalo_sc', providers=['CPUExecutionProvider'])
app.prepare(ctx_id=0, det_size=(160, 160))  # smaller = faster

cap = cv2.VideoCapture(0)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
cap.set(cv2.CAP_PROP_FPS, 60)  # request higher FPS from camera

faces = []  # cache faces between frames
frame_count = 0

scale_x, scale_y = 1, 1  # add this line
frame_count = 0

prev_time = time.time()

while True:
    ret, frame = cap.read()
    if not ret:
        break

    # Only run detection every 4 frames, draw cached results on others
    if frame_count % 4 == 0:
        small = cv2.resize(frame, (320, 240))  # detect on smaller frame
        faces = app.get(small)
        scale_x = frame.shape[1] / 320
        scale_y = frame.shape[0] / 240

    for face in faces:
        box = face.bbox.astype(int)
        # Scale box back up to full frame size
        x1 = int(box[0] * scale_x)
        y1 = int(box[1] * scale_y)
        x2 = int(box[2] * scale_x)
        y2 = int(box[3] * scale_y)
        cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
        cv2.putText(frame, f"{face.det_score:.2f}", (x1, y1 - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

    # Show FPS
    curr_time = time.time()
    fps = 1 / (curr_time - prev_time)
    prev_time = curr_time
    cv2.putText(frame, f"FPS: {fps:.1f}", (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

    cv2.imshow('Face Detector', frame)
    frame_count += 1

    if cv2.waitKey(1) & 0xFF == ord('q'):
        break

cap.release()
cv2.destroyAllWindows()